...
Object = "{456334B9-D052-4643-8F5F-2326B24BE316}#6.95#0"; "UAPvouchercontrol85.ocx"
...
'功能权限
Private Const AuthBrowse = "FSFY02010101"                  '浏览
Private Const AuthAdd = "FSFY02010102"                     '新增
Private Const AuthModify = "FSFY02010103"                  '修改
Private Const AuthDelete = "FSFY02010104"                  '删除
Private Const AuthVerify = "FSFY02010105"                  '审核
Private Const AuthUnVerify = "FSFY02010106"                '弃审
Private Const AuthOpen = "FYSL02050510"                    '打开
Private Const AuthClose = "FYSL02050511"                   '关闭
Private Const AuthPrint = "FSFY02010107"                   '打印
Private Const AuthOut = "FSFY02010108"                     '输出
Private Const AuthProrefer = "FYSL02050512"                '参照
Private Const AuthProrefer1 = "FYSL02050513"                '参照
...
'    End If
    ...
    m_bTrans = True
     ...
     '删除字表
    g_Conn.Execute "delete from " & DetailsTable & " where " & HeadPKFld & "=" & lngVoucherID
...
'
'        strSql = " update HY_FYSL_Contract set totalpaymoney=isnull(totalpaymoney,0)-" & Val(Voucher.bodyText(i, "colpriced")) & " where  ccode= '" & Voucher.bodyText(i, "concode") & "'"
'       g_Conn.Execute strSql
'
'
'        strSql = " update HY_FYSL_Payment set payamount=isnull(payamount,0)-" & Val(Voucher.bodyText(i, "colpriced")) & ",totalpaymoney=isnull(totalpaymoney,0)-" & Val(Voucher.bodyText(i, "colpriced")) & " where  ccode= '" & Voucher.bodyText(i, "palcode") & "'"
'        g_Conn.Execute strSql
'
'       Next i
'
...
Private Sub ExecSubModify()
    ...
    Voucher.VoucherStatus = VSeEditMode
     ...
     moneytmp = Null2Something(Voucher.headerText("progressmoney"), 0)
     numbertmp = Null2Something(Voucher.headerText("progressqty"), 0)
...
CHECK:
        For i = 1 To Voucher.BodyRows
            ...
            If Trim(Voucher.bodyText(i, "budgetcode")) = "" Then
                Voucher.row = i
                Voucher.DelLine
                GoTo CHECK
            End If
    ...
    Else
        ...
        For i = 1 To BodyData.Count
            '12.0表体扩展自定义项-update
            Set oDomBody = Voucher.GetLineDom(i)
            Set oBodyElement(i - 1) = oDomBody.selectSingleNode("//z:row")
...
Private Sub InitForm()
    ...
    pageCount = VchSrv.GetPageCount(g_Conn, gstrCardNumber, HeadPKFld, Replace(sAuth_ALL, "HY_FSFY_Costbudget.id", "V_HY_FSFY_Costbudget.id"))
    '由列表进入
    If CLng(sID) <> 0 Then
    ...
    Call InitToolBar(mologin, "HY_FSFY_Costbudget001", Toolbar, UFToolbar, Me.Voucher)
    Call UFToolbar.InitExternalButton("Costbudget001", mologin.OldLogin)
            ...
            Case "Prorefer2"
                ...
                If ReferVoucheng Then
                   ...
                   ' Me.Toolbar.Buttons("Prorefer2").Enabled = False
                    Me.Toolbar.Buttons("Prorefer").Enabled = False
                    Voucher.EnableHead "isreed", True
                    Me.UFToolbar.RefreshEnable
                Else
...
Private Sub ExecSubConfirm()
    ...
    End If
     ...
     sql = "insert into HY_FSFY_budgetlists(id,ddate,budgetcode, budcaliber ,cdeptcode ,prensoncode ,ccustcode , fibudgets  , sbudgets ,tbudgets ,fobudgets,budgetbalance,soucetype ) " & _
           " select id, '" & g_oLogin.CurDate & "',  budgetcode, budcaliber ,cdeptcode ,prensoncode ,ccustcode , fibudgets  , sbudgets ,tbudgets ,fobudgets,ISNULL(fibudgets,0)+ ISNULL(sbudgets,0)+ISNULL(tbudgets,0)+ISNULL(fobudgets,0),'预算单' " & _
          " From HY_FSFY_Costbudgets where  id='" & lngVoucherID & "'"
...
Private Sub ExecSubCancelconfirm()
    ...
    g_Conn.Execute "update " & MainTable & " set " & StrcHandler & "=null , " & StrdVeriDate & "=null , " & StriStatus & "=1 " & vbCrLf & _
     ...
     '更新
        ...
        sql = " delete  HY_FSFY_budgetlists where  id='" & lngVoucherID & "' and soucetype='预算单'"
       g_Conn.Execute sql
...
'    End If
    '部门
    If Voucher.headerText("coldepartcode") <> "" Then
        sql = "SELECT distinct coldepartcode  from HY_FSFY_Costbudget a left join department b on a.coldepartcode=b.cdepcode where a.coldepartcode='" & Voucher.headerText("coldepartcode") & "' "
        sql = sql & IIf(sAuth_depW = "", "", " and b.cdepcode in (" & sAuth_depW & ")")

        Set rs = New ADODB.Recordset
        rs.Open sql, g_Conn
        If rs.EOF Or rs.BOF Then
            MsgBox GetStringPara(("U8.DZ.JA.Res2050"), Voucher.headerText("coldepartcode")), vbInformation, GetString("U8.DZ.JA.Res030")
            Check_Auth = False
            Exit Function
        End If
    End If
...
Public Function processdataforcon() As Boolean
    ...
    strSql = "select 'Y' as selcol, * from  V_HY_FSFY_Costbudget_refer  where  id ='" & conid & "'"
    Set rs = New ADODB.Recordset
