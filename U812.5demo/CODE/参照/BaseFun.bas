...
Public Function VoucherIsCreate(ByVal lngID As Long) As Boolean
    ...
    tmpstr = "select supengcode from HY_FYSL_Projectrelease with(nolock) where  supengcode in (select ccode from HY_FYSL_Projectrelease with(nolock) where id = " & lngID & ")"
    tmpRst.Open tmpstr, g_Conn, adOpenStatic, adLockReadOnly
...
'    Else
'        tmpstr = " select * from V_HY_DZ_BorrowOutInAll " & _
'               " where djly='借出借用单'  and UAP_VoucherTransform_Rootid is not null" & _
'               " and UAP_VoucherTransform_Sourceid in (select autoid from hy_dz_borrowouts where id = " & lngID & ")"
'        tmprst.Close    'if tmprst.State =
'        tmprst.Open tmpstr, g_Conn, adOpenStatic, adLockReadOnly
'        If tmprst.RecordCount > 0 Then
'            VoucherIsCreate = True
'        Else
...
'        If tmpRst.State = adStateOpen Then tmpRst.Close
'        tmpRst.Open tmpstr, g_Conn
