...
Public Function GetMaxIDs()
    ...
    With cmd
        ...
        .Parameters.Append .CreateParameter("RemoteId", adVarChar, adParamInput, 2, "00")
        .Parameters.Append .CreateParameter("cAcc_Id", adVarChar, adParamInput, 100, g_oLogin.cAcc_Id)
        .Parameters.Append .CreateParameter("cVouchType", adVarChar, adParamInput, 50, "HY_FSFY_Costbudgets")
        .Parameters.Append .CreateParameter("iAmount", adBigInt, adParamInput, 100, 1)
        .Parameters.Append .CreateParameter("iFatherId", adBigInt, adParamOutput, 100, sID)
        .Parameters.Append .CreateParameter("iChildId", adBigInt, adParamOutput, 100, sAutoId)
        .Execute
...
Public Function GetFilterList(ologin As U8Login.clsLogin, Optional o_Filter As Object = Nothing, Optional sMenuPubFilter As String) As Boolean
    ...
    If o_Filter Is Nothing Then
        ...
        Else
            ...
            iRet = filterItf.OpenFilter(ologin, "", filtername, filtersa, sError, True)
            If iRet Then
...
'    End If
    ...
    Dim i As Long
    For i = 1 To objfltint.FilterList.Count

        If objfltint.FilterList(i).Name = "bSubmitted" Then

            If objfltint.FilterList(i).varValue = "0" Then
                strWhere = strWhere & " AND (isnull(iswfcontrolled,0)=1 and isnull(iverifystate,0)=0) "
            End If

            If objfltint.FilterList(i).varValue = "1" Then
                strWhere = strWhere & " AND (isnull(iswfcontrolled,0)=1 and isnull(iverifystate,0)<>0) "
            End If

            objfltint.FilterList(i).LngGrouptoken = 1
...
Private Function Refer_T(Voucher As Object, _
    ...
    Select Case sHeadItemName
            ...
            '单位
        Case LCase("custcode"), LCase("cCusAbbName")
    ...
    Case LCase("chdepartcode")
     ...
     Case LCase("procode")
            ...
            Ref.SetRWAuth "", "", False
                ...
                sqlStr = "select  distinct ccode from HY_FYSL_Measurement where   isnull(cHandler,'')<>'' and ccode not in (select isnull(procode,'') from  HY_FSFY_Costbudget  where isnull(procode,'')<>''   ) "

            If Ref.StrRefInit_SetColWidth(g_oLogin, False, "", sqlStr, "项目进度计量单号 ", "2000") = True Then
                Ref.Show
            End If
    ...
    If Not referpara.rstGrid.EOF Then
        ...
        Select Case sMetaItemName
            ...
            Case LCase("custcode"), LCase("cCusAbbName")
                ...
                If sMetaItemName = LCase("custcode") Then
    ...
    Select Case sMetaItemName
        ...
        Case "custcode"
            ...
            Else
                ...
                Else
                    Voucher.headerText("custcode") = strCellCode
        ...
        Case LCase("coldepartcode")
            ...
            If retvalue = "" Then
                Voucher.headerText("coldepartcode") = ""
            ...
            Else
                ...
                Else
                    Voucher.headerText("coldepartcode") = strCellCode
        ...
        Case LCase("procode")
            ...
            sql = "SELECT cCode from HY_FYSL_Measurement  where    cCode='" & retvalue & "'   and  id<>'" & lngVoucherID & "'"
            Set rs = New ADODB.Recordset
            rs.Open sql, g_Conn, 1, 1
...
Public Function VoucherbodyBrowUser(Voucher As Object, ByVal row As Long, ByVal Col As Long, sRet As Variant, referpara As UAPVoucherControl85.ReferParameter)
    ...
    If LCase(sMetaItemXML) Like "cdefine*" Or LCase(sMetaItemXML) Like "cbdefine*" Then
        ...
        End If
    '计划单号
    ElseIf LCase(sMetaItemXML) = "palcode" Then
    ...
    referpara.Cancel = True
            ...
            sqlStr = "select  distinct ccode,ecustcode,cCusAbbName,consubject,consubname from V_HY_FSFY_Costbudget_refer1  where ecustcode= '" & Voucher.headerText("custcode") & "' and  consubject='" & Voucher.headerText("coldepartcode") & "'  "

            If Ref.StrRefInit_SetColWidth(g_oLogin, False, "", sqlStr, "计划单号,客户编码,客户简称,收款部门编码,收款部门", "2000,2000,2000,2000,2000") = True Then
                Ref.Show
            End If
         ...
         '单位
         ElseIf LCase(sMetaItemXML) = LCase("ccustcode") Then
    ...
    ElseIf LCase(sMetaItemXML) = LCase("cdeptcode") Then
            referpara.id = "Department_AA"
            referpara.ReferMetaXML = "<Ref><RefSet bAuth='" & IIf(bDep_ControlAuth, "1", "0") & "' authFunID='W' /></Ref>"
            '过滤条件
            referpara.RetField = "cdepcode"
         ...
         ElseIf LCase(sMetaItemXML) = "prensoncode" Then
            referpara.id = "Person_AA"
            referpara.RetField = "cpersoncode"
            referpara.sSql = " dPValidDate <='" & g_oLogin.CurDate & "' and  isnull(dPInValidDate ,'2099-12-31') >='" & g_oLogin.CurDate & "' "
    
    '设计单号
    ElseIf LCase(sMetaItemXML) = "budgetcode" Then
            ...
            sqlStr = "select  distinct budgetcode,budgetname from HY_FSFY_Probudget "

            If Ref.StrRefInit_SetColWidth(g_oLogin, False, "", sqlStr, "预算项目编码,预算项目名称", "2000,2000") = True Then
                Ref.Show
            End If

            If Not (Ref.recmx Is Nothing) Then
                sRet = Ref.recmx("budgetcode")
            End If
    ...
    '合同
    ElseIf LCase(sMetaItemXML) = "concode" Then
        referpara.Cancel = True
            Set Ref = New UFReferC.UFReferClient
            Ref.SetLogin g_oLogin
            ...
            sqlStr = "select  distinct ccode,cname  from V_HY_FSFY_Costbudget_refer   "

            If Ref.StrRefInit_SetColWidth(g_oLogin, False, "", sqlStr, "合同编码,合同名称", "2000,4000") = True Then
                Ref.Show
            End If
...
Public Function ReferVouch() As Boolean
    ...
    clsReferVoucher.pageSize = 100                          '默认分页大小
    clsReferVoucher.strMainKey = "ID"                      '主表唯一主键，作为和子表关联的依据
    clsReferVoucher.strDetailKey = ""                '子表唯一主键
    clsReferVoucher.FrmCaption = "参照设计清单"
    clsReferVoucher.FilterKey = "PD010301MOMRef"
    clsReferVoucher.FilterSubID = "EF"


    clsReferVoucher.HeadKey = "PD010301MOMRef"               '主表的列信息 AA_ColumnDic
    ...
    clsReferVoucher.MainDataSource = "EF_V_HZDesignListMOMRef"    '主表数据源视图
    clsReferVoucher.DetailDataSource = ""    '子表数据源试图
    clsReferVoucher.DefaultFilter = ""           '默认过滤条件
    
'    clsReferVoucher.strAuth = Replace(sMakeAuth_ALL, "HY_FSFY_Costbudget.id", "V_HY_FSFY_Costbudget_refer2.id")    '数据权限SQL串, 这里不支持仓库权限控制
    ...
    clsReferVoucher.strCheckFlds = ""                 '"cType,bObjectCode"    '
    clsReferVoucher.strCheckMsg = ""
...
''参照单据
Public Function ReferVouch2() As Boolean
    ...
    clsReferVoucher.strMainKey = "autoid"                      '主表唯一主键，作为和子表关联的依据
    clsReferVoucher.strDetailKey = ""                '子表唯一主键
    clsReferVoucher.FrmCaption = "参照项目需求计划"
    clsReferVoucher.FilterKey = "参照项目需求计划"                '"借出借用单参照"                  '过滤器名称 SA26
    clsReferVoucher.FilterSubID = "EP"


    clsReferVoucher.HeadKey = "V_List_EF_ProjectMRP"               '主表的列信息 AA_ColumnDic
    clsReferVoucher.BodyKey = ""               '子表的列信息 ,若设置只有表头时该属性置空
    '添加自定义按钮
    'clsReferVoucher.Buttons = "<root><row type='toolbar' name='tlbTest' caption='自动匹配' index='26' /></root>"
    '多语时
    'strButtons ="<root><row type='toolbar' name='tlbTest' caption='{res:U8.SA.xsglsql_2.frmvouchrefers.automatching}' index='26' /></root>"
    'strButtons =ReplaceResId("<root><row type='toolbar' name='tlbTest' caption='{res:U8.SA.xsglsql_2.frmvouchrefers.automatching}' index='26' /></root>")
    clsReferVoucher.MainDataSource = "V_List_EF_ProjectMRP"    '主表数据源视图
    clsReferVoucher.DetailDataSource = ""    '子表数据源试图
    clsReferVoucher.DefaultFilter = "IsNULL(cVerifier,'')<>'' and IsNULL(cCloser,'')=''And IsNull(cBCloser,'')='' AND iwlyqty>0"           '默认过滤条件
...
'    rsbody.Open gDomReferBody
    dapprice = 0
    sourcetypetmp = ""
    bFirst = True
    If gDomReferHead.selectNodes("//z:row").Length > 0 Then
        
        For Each eleline In gDomReferHead.selectNodes("//z:row")
            If eleline.getAttribute("selcol") = "Y" Then
                
                Voucher.AddLine Voucher.BodyRows + 1
'                Voucher.bodyText(Voucher.BodyRows, "citem_class") = Null2Something(eleline.getAttribute("citem_class"))
'                Voucher.bodyText(Voucher.BodyRows, "citem_name") = Null2Something(eleline.getAttribute("citem_cname"))
'                Voucher.bodyText(Voucher.BodyRows, "citemcode") = Null2Something(eleline.getAttribute("citemcode"))
'                Voucher.bodyText(Voucher.BodyRows, "citemname") = Null2Something(eleline.getAttribute("citemname"))
'                Voucher.bodyText(Voucher.BodyRows, "cdefine22") = Null2Something(eleline.getAttribute("id"))
'                Voucher.bodyText(Voucher.BodyRows, "cdefine23") = Null2Something(eleline.getAttribute("autoid"))
'                Voucher.bodyText(Voucher.BodyRows, "cbmemo") = Null2Something(eleline.getAttribute("cbmemo"))
'                Voucher.bodyText(Voucher.BodyRows, "cdefine31") = Null2Something(eleline.getAttribute("cpmrpcode"))
'                Voucher.bodyText(Voucher.BodyRows, "cdefine32") = Null2Something(eleline.getAttribute("ccode"))
                
                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DInvCode", Null2Something(eleline.getAttribute("stockcode")), 0
                If Voucher.bodyText(Voucher.BodyRows, "DInvCode") <> "" Then
                    If Voucher.bodyText(Voucher.BodyRows, "DBStrFree_1") = -1 Then
                        Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DInvFree_1", Null2Something(eleline.getAttribute("projectnumber")), 0
                    End If
                    If Voucher.bodyText(Voucher.BodyRows, "DBStrFree_2") = -1 Then
                        Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DInvFree_2", Null2Something(eleline.getAttribute("cabinetnumber")), 0
                    End If
                    If Voucher.bodyText(Voucher.BodyRows, "DBStrFree_3") = -1 Then
                        Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DInvFree_3", "空", 0
                    End If
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DQty", Null2Something(eleline.getAttribute("iwmqty")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine14", Null2Something(eleline.getAttribute("projectname")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine15", Null2Something(eleline.getAttribute("cabinetname")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine17", Null2Something(eleline.getAttribute("reservename")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DDefine_28", Null2Something(eleline.getAttribute("cabinetid")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine14", Null2Something(eleline.getAttribute("cabinetnumber")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine15", Null2Something(eleline.getAttribute("cabinetname")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine24", Null2Something(eleline.getAttribute("projectnumber")), 0
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cbdefine25", Null2Something(eleline.getAttribute("projectname")), 0
                    Dim scbm As String
                    scbm = Getscbm(g_oLogin.cUserId)
                    If scbm <> "" Then
                        Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DMDeptCode", scbm, 0
                    End If
                    Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "DStartDate", g_oLogin.CurDate, 0
                End If
            End If
        Next
    End If
'    If gDomReferBody.selectNodes("//z:row").Length > 0 Then
'
'        For Each eleline In gDomReferBody.selectNodes("//z:row")
'            If eleline.getAttribute("selcol") = "Y" Then
'
'                Voucher.AddLine Voucher.BodyRows + 1
'                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "b_cinvcode", Null2Something(eleline.getAttribute("b_cinvcode")), 0
'                Voucher.bodyText(Voucher.BodyRows, "cpart") = Null2Something(eleline.getAttribute("cpart"))
'                Voucher.bodyText(Voucher.BodyRows, "coutsourced") = Null2Something(eleline.getAttribute("coutsourced"))
'
'                Voucher.bodyText(Voucher.BodyRows, "iunitqty") = Null2Something(eleline.getAttribute("iunitqty"))
'                Voucher.bodyText(Voucher.BodyRows, "iunitqtyold") = Null2Something(eleline.getAttribute("iunitqty"))
'                Voucher.bodyText(Voucher.BodyRows, "iqtyold") = Null2Something(eleline.getAttribute("iqty"))
'                Voucher.bodyText(Voucher.BodyRows, "iqty") = Null2Something(eleline.getAttribute("iqty"))
'
'            End If
'        Next
'    End If
...
'参照生单处理
Public Function ProcessData2(Voucher As Object, Optional Form As Object = Null)
...
'    rsbody.Open gDomReferBody
    ...
    If gDomReferHead.selectNodes("//z:row").Length > 0 Then
        ...
        For Each eleline In gDomReferHead.selectNodes("//z:row")
            If eleline.getAttribute("selcol") = "Y" Then
                cvouchid = Null2Something(eleline.getAttribute("cvouchid"))
                Voucher.AddLine Voucher.BodyRows + 1
'                Voucher.bodyText(Voucher.BodyRows, "citem_class") = Null2Something(eleline.getAttribute("citem_class"))
'                Voucher.bodyText(Voucher.BodyRows, "citem_name") = Null2Something(eleline.getAttribute("citem_cname"))
'                Voucher.bodyText(Voucher.BodyRows, "citemcode") = Null2Something(eleline.getAttribute("citemcode"))
'                Voucher.bodyText(Voucher.BodyRows, "cname") = Null2Something(eleline.getAttribute("citemname"))
                Voucher.bodyText(Voucher.BodyRows, "cdefine22") = Null2Something(eleline.getAttribute("id"))
                Voucher.bodyText(Voucher.BodyRows, "cdefine23") = Null2Something(eleline.getAttribute("autoid"))
                Voucher.bodyText(Voucher.BodyRows, "cdefine31") = Null2Something(eleline.getAttribute("ccode"))
                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "cinvcode", Null2Something(eleline.getAttribute("b_cinvcode")), 0
                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "iquantity", Null2Something(eleline.getAttribute("iwlyqty")), 0
                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "citem_class", Null2Something(eleline.getAttribute("citem_class")), 0
                Voucher.SimulateInput SectionsConstants.sibody, Voucher.BodyRows, "citemcode", Null2Something(eleline.getAttribute("citemcode")), 0
            End If
        Next
    End If
'    If gDomReferBody.selectNodes("//z:row").Length > 0 Then
...
Public Sub GlobalInit(login As clsLogin)
    ...
    '变量初始化
    gstrCardNumberlist = "FSFY002"
    gstrCardNumber = "FSFY001"

    '数据表定义
    MainTable = "HY_FSFY_Costbudget"                          '单据主表
    DetailsTable = "HY_FSFY_Costbudgets"                      '单据字表
    HeadPKFld = "ID"                                       '主表主键字段
    MainView = "V_HY_FSFY_Costbudget"                         '表头视图
    DetailsView = "V_HY_FSFY_Costbudgets"                     '表体视图
    VoucherList = "V_HY_FSFY_Costbudget_list"                  '列表视图
...
'写采购类单据
Public Function WritePUBill(ByRef oDomHead As DOMDocument, ByRef oDomBody As DOMDocument, conn As Object, login As clsLogin, VoucherType As String, sBillType As String) As Boolean
    On Error GoTo ErrHandler
    ...
    For i = 1 To rsize
        ...
        oNode.setAttribute "cvencode", r!cvencode          '  cVenCode 供应商编码  varchar 20  True
        oNode.setAttribute "cpersoncode", Null2Something(r!cpersoncode)    '  cPersonCode 业务员编码  varchar 20  True
        oNode.setAttribute "cdepcode", Null2Something(r!cdepcode)    '  cDepCode 部门编码  varchar 12  True
        oNode.setAttribute "cptcode", ""                   '  cPTCode 采购类型编码  varchar 2  True
        '取供应商信息
        strSql = "select cVenAddress,cVenIType from vendor where cVenCode='" & r!cvencode & "'"
        rs.Open strSql, conn, 1, 1
        oNode.setAttribute "carrivalplace", Null2Something(rs!cVenAddress)    '  cArrivalPlace 到货地址  varchar 200  True
        oNode.setAttribute "csccode", Null2Something(rs!cVenIType)    '  cSCCode 发运方式编码  varchar 2  True
        rs.Close

        oNode.setAttribute "cexch_name", Null2Something(r!cexch_name)    '  cexch_name 币种名称  varchar 8  True
        oNode.setAttribute "nflat", Null2Something(r!iExchRate)    '  nflat 汇率  float 8  True
        oNode.setAttribute "itaxrate", ""                  '  iTaxRate 表头税率  float 8  True
        oNode.setAttribute "cpaycode", ""                  '  cPayCode 付款条件编码  varchar 3  True
        oNode.setAttribute "icost", ""                     '  iCost 运费  money 8  True
        oNode.setAttribute "ibargain", ""                  '  iBargain 订金  money 8  True
        oNode.setAttribute "cmemo", GetString("U8.DZ.JA.Res990") & Time    '  cMemo 备注  varchar 60  True
        oNode.setAttribute "cstate", "1"                   '  cState 状态  tinyint 1  True
        oNode.setAttribute "cperiod", ""                   '  cPeriod 物料需求计划周期  varchar 12  True
        oNode.setAttribute "cmaker", login.cUserName       '  cMaker 制单人  varchar 20  True
        oNode.setAttribute "cverifier", ""                 '  cVerifier 审核人  varchar 20  True
        oNode.setAttribute "ccloser", ""                   '  cCloser 关闭人  varchar 20  True
        oNode.setAttribute "cbustype", strBusType          '  cBusType 业务类型  varchar 8  True

        oNode.setAttribute "idiscounttaxtype", "0"         '扣税类别
        ...
        ele.setAttribute "cpoid", POID
        ele.setAttribute "ufts", ""
        ele.setAttribute "dpodate", login.CurDate
        ...
        If rows > 0 Then
            ...
            For Each ele In eleList
                ...
                oNode.setAttribute "cinvcode", GetNodeAtrVal(ele, "cinvcode")
                '            iQuantity 数量  float 8  True icrdqty
                oNode.setAttribute "iquantity", CDbl(Format(GetNodeAtrVal(ele, "iquantity"), m_sQuantityFmt))
                '增加取价自由项       -xin 10-19
                sXML = "<?xml version='1.0' encoding='UTF-8'?><d><z "
                sXML = sXML & "cfree1='" & GetNodeAtrVal(ele, "cfree1") & "'"
                sXML = sXML & "cfree2='" & GetNodeAtrVal(ele, "cfree2") & "'"
                sXML = sXML & "cfree3='" & GetNodeAtrVal(ele, "cfree3") & "'"
                sXML = sXML & "cfree4='" & GetNodeAtrVal(ele, "cfree4") & "' "
                sXML = sXML & "cfree5='" & GetNodeAtrVal(ele, "cfree5") & "' "
                sXML = sXML & "cfree6='" & GetNodeAtrVal(ele, "cfree6") & "' "
                sXML = sXML & "cfree7='" & GetNodeAtrVal(ele, "cfree7") & "' "
                sXML = sXML & "cfree8='" & GetNodeAtrVal(ele, "cfree8") & "' "
                sXML = sXML & "cfree9='" & GetNodeAtrVal(ele, "cfree9") & "' "
                sXML = sXML & "cfree10='" & GetNodeAtrVal(ele, "cfree10") & "' "
                sXML = sXML & "/></d>"

                '修改取价        -xin 10-19
                GetInvBuyPrice login, conn, m_SysInfor, r!cvencode, GetNodeAtrVal(ele, "cinvcode"), fUnitCost, GetNodeAtrVal(ele, "iquantity"), fTaxCost, fTaxRate, bTaxCost, sXML

                fUnitCost = Format(CDbl(IIf(IsNull(fUnitCost), 0, fUnitCost)), m_sPriceFmt)    '原币无税单价
                fTaxCost = Format(CDbl(IIf(IsNull(fTaxCost), 0, fTaxCost)), m_sPriceFmt)    '原币含税单价
                fTaxRate = Format(CDbl(IIf(IsNull(fTaxRate), 0, fTaxRate)), m_iRateFmt)    '税率


                oNode.setAttribute "cunitid", GetNodeAtrVal(ele, "ccomunitname")    '主计量单位
                If GetNodeAtrVal(ele, "inum") <> "" Then _
                        oNode.setAttribute "inum", Format(CDbl(GetNodeAtrVal(ele, "inum")), m_sPriceFmt)    '件数              '件数
                If GetNodeAtrVal(ele, "iinvexchrate") <> "" Then _
                        oNode.setAttribute "iinvexchrate", Format(CDbl(GetNodeAtrVal(ele, "iinvexchrate")), m_sPriceFmt)    '换算率

                'bTaxCost 价格标准  bit 1  True
                oNode.setAttribute "btaxcost", IIf(bTaxCost, 1, 0)
                If bTaxCost Then                           '含税
                    fUnitCost = fTaxCost / (1 + fTaxRate / 100)
                Else                                       '无税
                    fTaxCost = fUnitCost * (1 + fTaxRate / 100)
                End If

                oNode.setAttribute "iunitprice", Format(CDbl(fUnitCost), m_sPriceFmt)    '            iUnitPrice 原币无税单价  float 8  True
                oNode.setAttribute "imoney", Format(fUnitCost * Val(GetNodeAtrVal(ele, "iquantity")), m_sPriceFmt)    '            iMoney 原币无税金额  money 8  True
                fTaxRate = CDbl(Format(IIf(IsNull(fTaxRate), 0, fTaxRate), m_iRateFmt))    '            iTax 原币税额  money 8  True
                oNode.setAttribute "itax", Format(CDbl(fUnitCost) * Val(GetNodeAtrVal(ele, "iquantity")) * fTaxRate / 100, m_sPriceFmt)

                oNode.setAttribute "isum", Format(fTaxCost * Val(GetNodeAtrVal(ele, "iquantity")), m_sPriceFmt)    '            iSum 原币价税合计  money 8  True
                '            iDisCount 原币折扣额  money 8  True

                oNode.setAttribute "inatunitprice", Format(CDbl(fUnitCost), m_sPriceFmt)    '            iNatUnitPrice 本币无税单价  float 8  True
                oNode.setAttribute "inatmoney", Format(fUnitCost * Val(GetNodeAtrVal(ele, "iquantity")), m_sPriceFmt)    '            iNatMoney 本币无税金额  money 8  True
                oNode.setAttribute "inattax", Format(CDbl(fUnitCost) * Val(GetNodeAtrVal(ele, "iquantity")) * fTaxRate / 100, m_sPriceFmt)    '            iNatTax 本币税额  money 8  True
                oNode.setAttribute "inatsum", Format(fTaxCost * Val(GetNodeAtrVal(ele, "iquantity")), m_sPriceFmt)    '            iNatSum 本币价税合计  money 8  True
                '            iNatDisCount 本币折扣额  money 8  True

                oNode.setAttribute "darrivedate", login.CurDate    '            dArriveDate 计划到货日期  datetime 8  True
                oNode.setAttribute "ipertaxrate", GetNodeAtrVal(ele, "itaxrate")    '            iPerTaxRate 税率  userdecimal 13  True
                oNode.setAttribute "itaxprice", Format(CDbl(fTaxCost), m_sPriceFmt)    '            iTaxPrice 原币含税单价  userdecimal 13  True
                oNode.setAttribute "cfree1", GetNodeAtrVal(ele, "cfree1")    '            cFree1 存货自由项1  varchar 20  True
                oNode.setAttribute "cfree2", GetNodeAtrVal(ele, "cfree2")    '            cFree2 存货自由项2  varchar 20  True
                oNode.setAttribute "cfree3", GetNodeAtrVal(ele, "cfree3")    '            cFree3 存货自由项3  varchar 20  True
                oNode.setAttribute "cfree4", GetNodeAtrVal(ele, "cfree4")    '            cFree4 存货自由项4  varchar 20  True
                oNode.setAttribute "cfree5", GetNodeAtrVal(ele, "cfree5")    '            cFree5 存货自由项5  varchar 20  True
                oNode.setAttribute "cfree6", GetNodeAtrVal(ele, "cfree6")    '            cFree6 存货自由项6  varchar 20  True
                oNode.setAttribute "cfree7", GetNodeAtrVal(ele, "cfree7")    '            cFree7 存货自由项7  varchar 20  True
                oNode.setAttribute "cfree8", GetNodeAtrVal(ele, "cfree8")    '            cFree8 存货自由项8  varchar 20  True
                oNode.setAttribute "cfree9", GetNodeAtrVal(ele, "cfree9")    '            cFree9 存货自由项9  varchar 20  True
                oNode.setAttribute "cfree10", GetNodeAtrVal(ele, "cfree10")    '            cFree10 存货自由项10  varchar 20  True

                oNode.setAttribute "bGsp", "0"             '            bGsp 是否质检  bit 1  True
                oNode.setAttribute "sotype", GetNodeAtrVal(ele, "sotype")    ' 需求跟踪方式
                ...
                oNode.setAttribute "bgsp", "0"
                'iinvmpcost 最高进价
                '增加供应商 最高进价 逻辑  2008-10-24 xin
                RSTOP.Open "select fTopCost from VenAndInv where cInvCode ='" & GetNodeAtrVal(ele, "cinvcode") & "'and  cVenCode='" & r!cvencode & "'", conn
                If Not RSTOP Is Nothing Then
                    If Not RSTOP.EOF Then
                        If Not IsNull(RSTOP!ftopcost) Then
                            oNode.setAttribute "iinvmpcost", Null2Something(RSTOP!ftopcost)
                        Else
                            oNode.setAttribute "iinvmpcost", ""
                        End If
                    Else
                        oNode.setAttribute "iinvmpcost", ""
                    End If
        ...
        Else
            lrows = 0
            strSql = "update " & MainTable & " set " & StriStatus & "=3,downstreamcode='" & POID & _
                    "'," & StrIntoUser & "='" & login.cUserId & "'," & StrdIntoDate & "='" & login.CurDate & "' where id= '" & r.Fields("ID") & "' and CONVERT(nchar,CONVERT(money,ufts),2)='" & r!ufts & "'"
            conn.Execute strSql, lrows
            If True Then                                   '自动审核
                errMsg = mVouchCO.ConfirmPO(CGHead)
                If errMsg <> "" Then
                    MsgBox GetString("U8.DZ.JA.Res1010") & errMsg, vbInformation, GetString("U8.DZ.JA.Res030")
                End If
            End If
        End If
        If lrows = 0 Then
    ...
    '   FrmMsgBox.Text1 = "成功生成 " & voucherSuccSize & " 张采购订单" & IIf(voucherSuccSize > 0, "，单号 " & vouchID, "") & vbCrLf & voucherErrMsg
    FrmMsgBox.Show 1
    'MsgBox "生成 " & vouchID & " 号销售订单成功!", vbInformation, pustrMsgTitle
    'End If
    Set r = Nothing
...
Public Function WriteSCBill(ByRef oDomHead As DOMDocument, ByRef oDomBody As DOMDocument, conn As Object, login As clsLogin, VoucherType As String, sBillType As String) As Boolean
    ...
    For i = 1 To rsize
        ...
        oNode.setAttribute "cwhcode", Null2Something(r!cwhcode)    '仓库编号 (必需字段)
        oNode.setAttribute "ddate", Date                   '单据日期
        oNode.setAttribute "crdcode", ""                   '收发类别
        oNode.setAttribute "btransflag", "0"               '是否传递
        oNode.setAttribute "cmaker", login.cUserName       '制单人
        oNode.setAttribute "cbustype", "其他入库"              '业务类型
        oNode.setAttribute "inetlock", "0"                 '现无用
        oNode.setAttribute "brdflag", "1"                  '收发标识
        oNode.setAttribute "cvouchtype", "08"              '单据类型(其他入库单)
        oNode.setAttribute "csource", GetString("U8.DZ.JA.Res030")    '来原来据
        ...
        oNode.setAttribute "cmemo", GetString("U8.DZ.JA.Res840") & Time    '备注
        oNode.setAttribute "iexchrate", Null2Something(r!iExchRate)    '汇率
        oNode.setAttribute "cexch_name", Null2Something(r!cexch_name)    '币种
        ...
        If rows > 0 Then
            ...
            For Each ele In eleList
                ...
                oNode.setAttribute "cinvcode", GetNodeAtrVal(ele, "cinvcode")    '存货编码
                ...
                oNode.setAttribute "cfree8", GetNodeAtrVal(ele, "cfree8")
                ...
                oNode.setAttribute "iquantity", Format(CDbl(GetNodeAtrVal(ele, "iquantity")), m_sQuantityFmt)    '数量
                ...
                If GetNodeAtrVal(ele, "itaxunitprice") <> "" Then _
                        oNode.setAttribute "iunitcost", Format(CDbl(GetNodeAtrVal(ele, "itaxunitprice")), m_sPriceFmt)    '本币单价
                If GetNodeAtrVal(ele, "inatmoney") <> "" Then _
                        oNode.setAttribute "iprice", Format(CDbl(GetNodeAtrVal(ele, "inatmoney")), m_sPriceFmt)    '本币金额
                If GetNodeAtrVal(ele, "itaxrate") <> "" _
                        Then oNode.setAttribute "itaxrate", Format(CDbl(GetNodeAtrVal(ele, "itaxrate")), m_iRateFmt)    '税率
        ...
        If pco.Insert(VoucherType, SCHead, SCBody, Postion, errMsg, conn, rdID, domMsg, False) = False Then
        ...
        Else
            lrows = 0
            Set rs = conn.Execute("select ccode from rdrecord where id=" & rdID)
            If Not rs Is Nothing Then
                If Not rs.EOF Then rdID = Null2Something(rs(0))
            End If
            strSql = "update " & MainTable & " set " & StriStatus & "=3,downstreamcode='" & rdID & _
                    "'," & StrIntoUser & "='" & login.cUserId & "'," & StrdIntoDate & "='" & login.CurDate & "' where id= '" & r.Fields("ID") & "' and CONVERT(nchar,CONVERT(money,ufts),2)='" & r!ufts & "'"
            conn.Execute strSql, lrows
            If True Then                                   '自动审核
                'Function Verify(sVouchType As String, VouchId, errMsg As String, [cnnFrom As Connection], [TimeStamp], [domMsg As DOMDocument], [bCheck As Boolean = True], [bBeforCheckStock As Boolean = True], [bList As Boolean = False], [MakeWheres As Collection], [sWebXml As String], [oGenVouchIds As Dictionary]) As Boolean
                If pco.Verify(VoucherType, rdID, errMsg, conn) Then
                    MsgBox GetString("U8.DZ.JA.Res1040") & errMsg, vbInformation, GetString("U8.DZ.JA.Res030")
                End If
            End If
        End If
        If lrows = 0 Then
    ...
    If gDomReferHead.selectNodes("//z:row").Length > 0 Then
        ...
        For Each eleline In gDomReferHead.selectNodes("//z:row")
            If eleline.getAttribute("selcol") = "Y" Then
                ...
                Voucher.AddLine Voucher.BodyRows + 1
            If Null2Something(eleline.getAttribute("sourcetype")) = "参照合同" Then
                Voucher.headerText("custcode") = Null2Something(eleline.getAttribute("ecustcode"))
                Voucher.headerText("cCusAbbName") = Null2Something(eleline.getAttribute("cCusAbbName"))
                Voucher.headerText("cCusName") = Null2Something(eleline.getAttribute("cCusName"))
                sourcetypetmp = "参照合同"
            Else
                 Voucher.headerText("custcode") = Null2Something(eleline.getAttribute("designunits"))
                Voucher.headerText("cCusAbbName") = Null2Something(eleline.getAttribute("decabbname"))
                Voucher.headerText("cCusName") = Null2Something(eleline.getAttribute("descuname"))
                sourcetypetmp = "参照设计"
            End If
            
            Voucher.headerText("coldepartcode") = Null2Something(eleline.getAttribute("consubject"))
            Voucher.headerText("chdepname") = Null2Something(eleline.getAttribute("consubname"))
            
            
            Voucher.bodyText(Voucher.BodyRows, "appprice") = Null2Something(eleline.getAttribute("appprice"))
            Voucher.bodyText(Voucher.BodyRows, "palcode") = Null2Something(eleline.getAttribute("ccode"))
             Voucher.bodyText(Voucher.BodyRows, "conname") = Null2Something(eleline.getAttribute("conname"))
            Voucher.bodyText(Voucher.BodyRows, "concode") = Null2Something(eleline.getAttribute("concode"))
            Voucher.bodyText(Voucher.BodyRows, "totalpaymoney") = Null2Something(eleline.getAttribute("totalpaymoney"))
            Voucher.bodyText(Voucher.BodyRows, "contolprice") = Null2Something(eleline.getAttribute("contolprice"))
            Voucher.bodyText(Voucher.BodyRows, "totalappmoney") = Null2Something(eleline.getAttribute("totalpaymoney"))
            Voucher.bodyText(Voucher.BodyRows, "conpaytolmoney") = Null2Something(eleline.getAttribute("conpaytolmoney"))
            Voucher.bodyText(Voucher.BodyRows, "proccode") = Null2Something(eleline.getAttribute("proccode"))
            Voucher.bodyText(Voucher.BodyRows, "proname") = Null2Something(eleline.getAttribute("proname"))
            Voucher.bodyText(Voucher.BodyRows, "engcode") = Null2Something(eleline.getAttribute("engcode"))
            Voucher.bodyText(Voucher.BodyRows, "engname") = Null2Something(eleline.getAttribute("engname"))
            
            
'            Voucher.bodyText(Voucher.BodyRows, "colpriced") = Val(Null2Something(eleline.getAttribute("contolprice"), 0)) - Val(Null2Something(eleline.getAttribute("totalpaymoney"), 0))
'            dapprice = dapprice + Val(Null2Something(eleline.getAttribute("contolprice"), 0)) - Val(Null2Something(eleline.getAttribute("totalpaymoney"), 0))
'            Voucher.headerText("colprice") = dapprice
'            Voucher.headerText("canamount") = dapprice
...
'获取默认生产部门
Private Function Getscbm(cuser As String) As String
    Dim strSql As String
    Dim rs As New ADODB.Recordset
    On Error GoTo hErr
    strSql = "select scbm from hr_hi_person where cpsn_num='" & cuser & "' "
    rs.Open strSql, g_Conn
