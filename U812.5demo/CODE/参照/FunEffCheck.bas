...
'    End If
'
'   If Voucher.headerText("colprice") = "" Then
'        MsgBox "收款金额不能为空", vbInformation, GetString("U8.DZ.JA.Res030")
'        ExecFunEffectiveCheck = False
'
'        Exit Function
'
...
Private Function ExecFunBodyCheck(Voucher As Object) As Boolean
    ...
    For Each bodyele In oDomBody.selectNodes("//z:row[@editprop != 'D']")
        ...
        '其他的校验根据实际业务而定
         ...
         If bodyele.getAttribute("cdeptcode") <> "" Then
        sql = "select dDepEndDate from department where cdepcode='" + bodyele.getAttribute("cdeptcode") + "'"
