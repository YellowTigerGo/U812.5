...
Begin VB.Form frmReferFPBGD 
   Caption         =   "生单列表"
   ClientHeight    =   6555
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   10965
   Icon            =   "frmReferFPBGD.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6555
   ScaleWidth      =   10965
   ...
   Begin ReferMakeVouch.ctlReferMakeVouch ctlReferMakeVouch1 
      DragIcon        =   "frmReferFPBGD.frx":058A
      Height          =   6405
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmReferFPBGD"
...
Public Cardnumber As String  '目标单据类型
...
Public Sub SetData(Optional headkey_val As String, Optional bAppend As Boolean = False, Optional Errstr As String)
    ...
    Dim i As Long
    Dim strVoucherType As String '正品报工单单据类型
    ...
    If isBody = False Then '刷新表头数据
        ...
        '废品报工单参照正品报工单时，只能参照对应类型的正品报工单
 
 
        If LCase(Source_Cardnum) = LCase("YXEF9131_refbg") Then
            Select Case LCase(Cardnumber)
                Case LCase("YXEF9131")
                    strVoucherType = " cvouchtype = 'YXEF9115' "
                Case LCase("YXEF9132")
                    strVoucherType = " cvouchtype = 'YXEF9116' "
                Case LCase("YXEF9133")
                    strVoucherType = " cvouchtype = 'YXEF9117' "
                Case LCase("YXEF9134")
                    strVoucherType = " cvouchtype = 'YXEF9118' "
                Case LCase("YXEF9135")
                    strVoucherType = " cvouchtype = 'YXEF9119' "
                Case LCase("YXEF9136")
                    strVoucherType = " cvouchtype = 'YXEF9120' "
            End Select
            
            If strVoucherType <> "" Then strSql = strSql & " and " & strVoucherType
        End If
...
Public Sub ButtonClick(ByVal cButtonId As String)
...
Function Init(m_login As U8Login.clsLogin, Cardnumbers As String, DBcon As ADODB.Connection, Frm As Object, Dest_Domh As DOMDocument, Dest_Domb As DOMDocument, Source_Cardnumber As String, Dest_Cardnumber As String) As Boolean
...
'    For i = 1 To ctlReferMakeVouch1.BodyList.rows - 1
'        '处理只能选择一条记录的问题 ccw
'        If ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("selcol")) = "Y" Then
'            k = i
'        End If
'
'        If (i <> k) And _
'                ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("selcol")) = "Y" Then
