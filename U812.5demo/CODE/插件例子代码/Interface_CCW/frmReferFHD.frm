...
Begin VB.Form frmReferFHD 
   Caption         =   "生单列表"
   ClientHeight    =   6555
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   10965
   Icon            =   "frmReferFHD.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6555
   ScaleWidth      =   10965
...
Public Sub SetData(Optional headkey_val As String, Optional bAppend As Boolean = False, Optional Errstr As String)
    ...
    Dim GetSQLWhere As String
    Dim i As Long
    Dim ccuscode As String
    Dim defHwhere As String
    Dim defBwhere As String
    
    
    If Not Head_filter.FilterList Is Nothing Then
        For i = 1 To Head_filter.FilterList.Count
            If LCase(Head_filter.FilterList.Item(i).FieldName) = LCase("t_ccuscode") Then
                ccuscode = Head_filter.FilterList.Item(i).varValue
                Head_filter.FilterList.Item(i).varValue = ""
                Head_filter.FilterList.Item(i).varValue2 = ""
            End If
        Next
    End If
    
    If ccuscode <> "" Then
        defHwhere = " id in ( select id from  " & Body_Source & "  where  t_ccuscode = '" & ccuscode & "' )"
        defBwhere = " and  t_ccuscode = '" & ccuscode & "'"
    End If
    ...
    If isBody = False Then '刷新表头数据
        ...
        Select Case cVouchType
            ...
            Case Else
                ...
                If Head_filter.GetSQLWhere <> "" Then
                        GetSQLWhere = Head_filter.GetSQLWhere
                    End If
        ...
        If GetSQLWhere <> "" Then strSql = strSql & "  and " & GetSQLWhere
        If defHwhere <> "" Then strSql = strSql & "  and " & defHwhere
...
Refreshbody:
        ...
        If Len(headkey_val) = 0 Then headkey_val = 0
        strSql = strSql & " and " & Me.head_key & " in (" & headkey_val & ") "
        If defBwhere <> "" Then strSql = strSql & defBwhere
...
'    If oRef.ColSetKeyBody = "" Then Exit SubDim key_val As String
Dim i As Long
    For i = 1 To ctlReferMakeVouch1.HeadList.rows - 1
'        If row = i Then'
''        key_val = key_val & ",'" & ctlReferMakeVouch1.HeadTextMatrix(row, ctlReferMakeVouch1.GetHeadColIndex(Me.head_key)) & "'"'
''        Else
'        Debug.Print "------" & ctlReferMakeVouch1.HeadTextMatrix(i, ctlReferMakeVouch1.GetHeadColIndex("selcol"))
       
'            ctlReferMakeVouch1.HeadList.TextMatrix(i, 0) = ""
'        End If

        '处理多选的问题 ccw
        If ctlReferMakeVouch1.HeadTextMatrix(i, ctlReferMakeVouch1.GetHeadColIndex("selcol")) = "Y" Then
            key_val = key_val & "'" & ctlReferMakeVouch1.HeadTextMatrix(i, ctlReferMakeVouch1.GetHeadColIndex(Me.head_key)) & "',"
        End If
    Next
    key_val = Left(key_val, Len(key_val) - 1)
...
Private Sub ctlReferMakeVouch1_onBodySelected(ByVal row As Long, sErr As String)
    BodySelected
End Sub
...
'            End If
            ...
            If Not ele.Attributes.getNamedItem("b_" & Attributes.nodeName) Is Nothing Then
                Attributes.Value = ele.Attributes.getNamedItem("b_" & Attributes.nodeName).nodeValue
            End If
...
Function Init(m_login As U8Login.clsLogin, DBcon As ADODB.Connection, Frm As frmReferFHD, Dest_Domh As DOMDocument, Dest_Domb As DOMDocument, Source_Cardnumber As String, Dest_Cardnumber As String) As Boolean
...
',处理只能在表体选择一个客户，一个业务类型
Public Sub BodySelected()
Dim i As Long
Dim t_ccuscode As String  '客户
Dim strBusType As String  '业务类型

Dim k As Long
    t_ccuscode = ""
    strBusType = ""
    For i = 1 To ctlReferMakeVouch1.BodyList.rows - 1
        '处理客户、业务类型不能多选的问题 ccw t_ccuscode
        If ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("selcol")) = "Y" And (t_ccuscode = "" Or strBusType = "") Then
            t_ccuscode = ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("t_ccuscode"))
            strBusType = ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("t_cbustype"))
            k = i
        End If
        
        If (i <> k) And _
                ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("selcol")) = "Y" And _
                ((t_ccuscode <> ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("t_ccuscode"))) Or _
                (strBusType <> ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("t_cbustype")))) Then
            MsgBox "生成发货单时只能选择一个客户和一个业务类型！"
            ctlReferMakeVouch1.BodyTextMatrix(i, ctlReferMakeVouch1.GetBodyColIndex("selcol")) = ""
        End If
    Next
